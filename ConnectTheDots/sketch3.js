...
let currentScale = 1;    // Will be calculated based on screen size
...
function draw() {
  ...
  if (isDrawingComplete) {
    ...
    fill(255);
    stroke(0);
    let eyeX = points[0].x - (10 * currentScale);
    let eyeY = points[0].y + (10 * currentScale);
  ...
  if (!isDrawingComplete) {
    ...
    fill(0, 150, 0);
    text('Drawing Complete!', width/2, instructionY);
  }
}
...
function touchEnded() {
  handleEnd(touches[0].x, touches[0].y);
  return false; // Prevent default
}
